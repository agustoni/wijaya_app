	...
	$scriptCreatePO = "
		...
		function createPOItem(lastNumber, idProjectItem = '', idItem = '', itemName = '', idSupplier = '', supplierName = '', qty = '', harga = ''){
			var poItemContent = $(`<tr id='projectitem-row_`+lastNumber+`'>
										...
										<td class='align-middle'>
											<!-- <label class='projectitem_item-ItemName'>`+itemName+`</label> -->
											<input class='form-control projectitem_item-ItemName' value='`+itemName+`'>
											
											<input class='form-control projectitem_item-IdItem' value='`+idItem+`'>
										</td>
										<td class='align-middle'>
											<!-- <label class='projectitem_input-SupplierName'>`+supplierName+`</label> -->
											<input class='form-control projectitem_input-SupplierName' value='`+idSupplier+`'>
											
											<input class='form-control projectitem_label-IdSupplier' value='`+idSupplier+`'>
										</td>
										<td class='align-middle'>
											<input class='form-control projectitem_input-Qty' value='`+qty+`'>
										</td>
										<td class='align-middle'>
											<input class='form-control projectitem_input-Harga' value='`+harga+`'>
										</td>
			...
			itemTypeaheadInit($('#project-item-list tbody').find('#projectitem-row_'+lastNumber+' .projectitem_item-ItemName'))
        ...
        function supplierTypeaheadInit(inputTypeahead){
            var src = new Bloodhound({
                ...
                queryTokenizer: Bloodhound.tokenizers.whitespace,
                remote: {
                    url: '".Yii::$app->urlManager->createUrl('project/get-supplier')."&q=%QUERY',
                    wildcard: '%QUERY'
                }
            })
