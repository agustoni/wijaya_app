    ...
    $('body').on('click', '#save_ItemCombined', function(){
        ...
        if(validate){
            if(confirm('Apakah Anda yakin?')){
                $.ajax({
                    type: 'POST',
                    url: _url+'backend/item/save-item-combined',
                    data: data,
                    contentType: false,
                    cache: false,
                    processData: false,
...
// ================== TYPEAHEAD ITEM PART ==================
    ...
    function deleteItemPart(idItemPart, el){
        $.ajax({
            type: 'POST',
            url: _url+'backend/item/delete-part-combined',
            data: {idItemPart},
            dataType:'json',
            async: true,
            beforeSend: function() {
    ...
    function itemPartTypeaheadInit(inputTypeahead){
        var src = new Bloodhound({
            ...
            remote: {
                url: _url+'backend/item/get-all-item?q=%QUERY',
                wildcard: '%QUERY'
            }
        })
...
// ================== TYPEAHEAD ITEM PARENT ==================
    ...
    function itemParentTypeaheadInit(){
        var src = new Bloodhound({
            ...
            queryTokenizer: Bloodhound.tokenizers.whitespace,
            remote: {
                url: _url+'backend/item/get-all-item-combined?q=%QUERY',
                wildcard: '%QUERY'
            }
        })
